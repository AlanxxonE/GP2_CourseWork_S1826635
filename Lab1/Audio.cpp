...
Audio::Audio()
{
	...
	if (device == NULL)
	{
		std::cout << "cannot open sound card" << std::endl; //check for errors regarding the sound card
	}
	...
	if (context == NULL)
	{
		std::cout << "cannot open context" << std::endl; //check for errors regarding the context
	}
...
Audio::~Audio()
{
	//deletes all audio sources and destroys the context
...
{
	//reads the Wave file in text format to detect all the different bit information based on the letters set up in each line
